...
public class AnimalTraits
{
    ...
    public Dictionary<Trait, RangedDouble> numericalTraits = new Dictionary<Trait, RangedDouble>();


    public AnimalTraits(Species species, double maxSize, double dietFactor, double nChildren, double infantFactor, double growthFactor, double speed, double heatTimer, double sightLength, double smellRadius, FCMHandler fcmHandler)
    {
        this.species = species;
        this.maxSize = new RangedDouble(maxSize, 0);
        this.dietFactor = new RangedDouble(dietFactor, 0, 1);
        this.nChildren = new RangedDouble(nChildren, 1);
    ...
    public (double, string)[] GetNumericalTraits()
    {
        ...
        traits[2] = (nChildren.GetValue(), "#children");
